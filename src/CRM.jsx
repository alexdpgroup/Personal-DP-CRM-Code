            ...
            <table>
              ...
              <tbody>
                {investors.map(inv => {
                  ...
                  // pick the most advanced stage across all records
                  ...
                  return (
                    <tr key={inv.name + inv.firm} onClick={() => setSelected(inv.records[0])}>
                      <td>
                    ...
                    const EditableCell = ({ field, isNum = true, align = "right" }) => (
                      isEditing(field)
                        ...
                        : <span
                          ...
                          >
                            {isNum && field !== "costPerShare" && field !== "fmvPerShare" ? fmtMoney(fin[field]) : fin[field]}
                          </span>
                    );
                    return (
                      <tr key={fin.id} style={{ background: "#fff" }}>
                        ...
                        <td style={{ textAlign: "right", fontSize: 12 }}>
                          {fin.fmvPerShare > 0 ? <EditableCell field="fmvPerShare" /> : <span style={{ color: "var(--ink-muted)" }}>N/A</span>}
                        </td>
                        <td style={{ textAlign: "right" }}>
...
function AddFundModal({ onClose }) {
  ...
  const handleSave = async () => {
    ...
    // Load current fund defs, add new one, save
    try {
      const currentDefs = await loadData("dp_fund_defs_v1", FUND_DEFS);
      const updated = [...currentDefs, newFund];
      await saveData("dp_fund_defs_v1", updated);
      
      alert(`Fund "${fullName}" created! Refresh the page to see it in the sidebar.`);
      onClose();
    } catch (err) {
      console.error("Error saving fund:", err);
      alert("Error creating fund. Please try again.");
    }
  };
